...
class AssignmentDeadlineVo
{
  ...
  private function validateAndCreateDateTime(string $datetimeString): DateTimeImmutable
  {
    if (empty(trim($datetimeString))) {
      throw new InvalidArgumentException("Assignment deadline string cannot be empty");
    }
    ...
    try {
      ...
      throw new InvalidArgumentException(
        sprintf('Invalid assignment deadline format "%s". Expected ISO 8601 format', $datetimeString)
      );
    }
    ...
    if ($datetime <= $now) {
      throw new InvalidArgumentException(
        sprintf('Assignment datetime "%s" must be in the future', $datetime->format('Y-m-d H:i:s'))
      );
    }
