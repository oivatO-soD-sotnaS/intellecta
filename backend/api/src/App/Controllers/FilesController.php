...
readonly class FilesController extends BaseController
{
  ...
  public function uploadFile(Request $request, Response $response): Response {
    return $this->handleErrors($request, function() use ($request, $response) {
      ...
      $this->validatorService->validateRequired($uploadedFiles, ['file']);
      
      $file = new FileVo($uploadedFiles['file']);
      $fileUrl = $this->uploadService->upload(
        $file->getExtension(), 
        $file->getContent()
      );
      ...
      LogService::info('/files/upload-file', "File uploaded successfully!");
      return $response;
    });
  }
}