// app/api/ai/classes/suggest/route.ts
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await client.responses.create({
      model: "gpt-4o-mini",
      instructions: `
Você é uma IA que ajuda a criar turmas dentro de uma plataforma de ensino chamada Intellecta.
...
- O JSON deve ter exatamente estes campos:
  {
    "name": "nome curto da turma (ex.: 3º ano A - Matemática)",
    "description": "descrição em 1 a 3 frases explicando foco, público e objetivo da turma"
  }
- Não explique o que está fazendo, não coloque comentários, não coloque markdown.
    ...
    console.error("[AI_CLASS_SUGGEST_ERROR]", error)
    return NextResponse.json(
