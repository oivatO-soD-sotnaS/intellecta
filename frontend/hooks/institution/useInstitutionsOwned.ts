...
import { mapApiInstitution, mapApiInstitutionSummary, normalizeList } from "@/types/institution.mappers";

export function useInstitutionsOwned() {
  return useQuery({
    queryKey: ["institutions", "owned"],
    queryFn: async () => {
      const data = await apiGet<ApiInstitutionSummary[] | { items: ApiInstitutionSummary[] }>("/api/institutions/owned");
